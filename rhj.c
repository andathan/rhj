...
Relation *BuildRelation(inbetween_results *inb,int rel_id,Relation *init_relation){
  ...
  new_relation->tuples = malloc(sizeof(tuple)*inb->joined[rel_id]);
  new_relation->num_tuples=inb->joined[rel_id];
  for(i=0;i<inb->joined[rel_id];i++){
    new_relation->tuples[i].key = i;
    new_relation->tuples[i].payload = init_relation->tuples[inb->inbetween[rel_id][i]].payload;
  }
  return new_relation;
}
